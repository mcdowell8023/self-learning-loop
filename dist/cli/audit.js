...
function loadAuditEvents(auditDir) {
...
function auditList(argv, auditDir, out, _err) {
    let typeFilter;
    let sinceFilter;
    let limit = 50;
    for (let i = 0; i < argv.length; i++) {
        const tok = argv[i];
        if (tok === '--type' && argv[i + 1])
            typeFilter = argv[++i];
        else if (tok.startsWith('--type='))
            typeFilter = tok.slice('--type='.length);
        else if (tok === '--since' && argv[i + 1])
            sinceFilter = argv[++i];
        else if (tok.startsWith('--since='))
            sinceFilter = tok.slice('--since='.length);
        else if (tok === '--limit' && argv[i + 1])
            limit = parseInt(argv[++i], 10) || 50;
        else if (tok.startsWith('--limit='))
            limit = parseInt(tok.slice('--limit='.length), 10) || 50;
    }
...
function auditReplay(argv, auditDir, out, err) {
...
function auditStats(argv, auditDir, out, _err) {
    let days = 30;
    for (let i = 0; i < argv.length; i++) {
        const tok = argv[i];
        if (tok === '--days' && argv[i + 1])
            days = parseInt(argv[++i], 10) || 30;
        else if (tok.startsWith('--days='))
            days = parseInt(tok.slice('--days='.length), 10) || 30;
    }
