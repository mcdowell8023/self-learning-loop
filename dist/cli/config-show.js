...
export async function runConfigShow(opts) {
    ...
    for (let i = 0; i < opts.argv.length; i++) {
        ...
        if (tok === '--json')
            jsonOutput = true;
        else if (tok === '--key' && opts.argv[i + 1])
            keyFilter = opts.argv[++i];
        else if (tok.startsWith('--key='))
            keyFilter = tok.slice('--key='.length);
        else if (tok === '-h' || tok === '--help') {
            out('Usage: openclaw-learn config show [--json] [--key <path>]\n\n' +
                'Options:\n' +
                '  --json          Output as JSON.\n' +
    ...
    try {
        ...
        err(`error: failed to load config: ${e.message}\n`);
...
function renderConfig(config, out, prefix = '') {
    for (const [key, value] of Object.entries(config)) {
        const fullKey = prefix ? `${prefix}.${key}` : key;
        if (value != null && typeof value === 'object' && !Array.isArray(value)) {
            out(`${fullKey}:\n`);
            renderConfig(value, out, fullKey);
        }
        else {
