...
export function parseStatusArgs(argv) {
    ...
    for (let i = 0; i < argv.length; i++) {
        ...
        if (a === '-h' || a === '--help')
            ...
            const v = argv[++i];
            if (!v)
                return { kind: 'error', code: 2, message: 'error: --scope requires a value' };
            scopeFilter = v.split(',').map((s) => s.trim()).filter(Boolean);
        }
        else if (a === '--state') {
            const v = argv[++i];
            if (!v)
                return { kind: 'error', code: 2, message: 'error: --state requires a value' };
            stateFilter = v.split(',').map((s) => s.trim()).filter(Boolean);
        }
        else if (a === '--db') {
            const v = argv[++i];
            if (!v)
                return { kind: 'error', code: 2, message: 'error: --db requires a value' };
            dbFlag = v;
        }
        else if (a === '--workspace') {
            const v = argv[++i];
...
function countTrials(c) {
    let n = 0;
    for (const inst of c.instances)
        n += inst.trial_results?.length ?? 0;
    return n;
}
...
function renderList(candidates, out) {
...
function renderDetail(c, out) {
    out(`Candidate ${c.candidate_id}\n`);
    out('─'.repeat(60) + '\n');
    out(`  state            : ${c.state}\n`);
    if (c.dormant_reason)
        out(`  dormant_reason   : ${c.dormant_reason}\n`);
    ...
    try {
        if (parsed.candidateId) {
            ...
            if (!candidate) {
                ...
                if (matches.length > 1) {
                    err(`error: id prefix '${idOrPrefix}' is ambiguous (${matches.length} matches):\n` +
        ...
        if (parsed.scopeFilter) {
            ...
            all = all.filter((c) => set.has(c.strategy.scope));
        }
