...
// ─── Helpers ────────────────────────────────────────
function defaultProjectPath(env) {
    ...
    try {
        ...
        try {
            ...
            return {
                ...
                message: `YAML parse failed at ${path}: ${err.message}`,
            };
        }
    }
    catch (err) {
        return {
            ...
            message: `Read failed at ${path}: ${err.message}`,
        };
    }
}
/** 深度合并：后者覆盖前者；数组直接替换。 */
...
function extractEnvOverrides(env) {
    const out = {};
...
function buildMerged(opts) {
    ...
    if (existsSync(opts.userConfigPath)) {
        ...
        if (!r.ok)
            return { ok: false, error_code: r.error_code, message: r.message };
...
function validate(raw) {
    try {
        ...
        return {
            ...
            message: zerr ? zerr.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('; ') : err.message,
            details: zerr?.issues,
        };
    }
}
function diffTopLevel(a, b) {
    const changed = [];
...
export function reloadConfig(opts) {
    if (!_singleton) {
        try {
            _singleton = ConfigLoader.load(opts ?? {});
            ...
            return { ok: false, error_code: 'config_read_error', message: err.message };
        }
    }
