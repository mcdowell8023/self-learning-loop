...
export const permissiveEvidenceStore = {
 ...
 */
export function aggregateL1(assertions, trials) {
    const hasCommandAssertion = assertions.some((a) => a.type === 'command_exit_code');
    let passed = 0;
    let failed = 0;
    let skipped = 0;
    let total = 0;
    for (const trial of trials) {
        for (const r of trial.assertions) {
            ...
            if (r.status === 'pass')
                passed++;
            else if (r.status === 'fail' || r.status === 'timeout' || r.status === 'error')
                failed++;
...
export function requiresSecureL1(assertions) {
    return assertions.some((a) => a.type === 'command_exit_code');
}
export function hasSecureL1Evidence(trials, store) {
    return trials.some((t) => {
        const ev = t.secure_l1_evidence;
        if (!ev)
            return false;
        if (ev.exit_code_verified !== true)
            return false;
        if (!['bwrap', 'firejail', 'docker'].includes(ev.sandbox_type))
            return false;
        if (typeof ev.execution_trace_hash !== 'string')
            return false;
        if (ev.execution_trace_hash.trim() === '')
            return false;
        if (!SHA256_RE.test(ev.execution_trace_hash))
            return false;
        if (!store.hasTrace(ev.execution_trace_hash))
            return false;
        return t.assertions.some((a) => a.type === 'command_exit_code' && a.status === 'pass');
    });
}
// ---------------------------------------------------------------------------
 ...
 */
export function evaluateL2(samples) {
 ...
 */
export function applyTruthTable(l1, l2, l3) {
...
// ---------------------------------------------------------------------------
export function computeConfidence(passRate, sampleSize, l2EffectSize) {
    if (sampleSize <= 0)
        return 0;
    const p = Math.max(0, Math.min(1, passRate));
    const n = sampleSize;
    const z = 1.96;
    const wilsonLower = (p + (z * z) / (2 * n) - z * Math.sqrt((p * (1 - p) + (z * z) / (4 * n)) / n)) /
        (1 + (z * z) / n);
...
export function evaluateCandidate(input) {
    const { candidate_id, assertions, trials, metrics, evidenceStore = permissiveEvidenceStore, l3 = 'skipped', now = () => new Date(), } = input;
    const l1 = aggregateL1(assertions, trials);
    const l2 = evaluateL2(metrics);
    const l3Status = l3 ?? 'skipped';
    ...
    if (secure_l1_required_triggered &&
    ...
    const confidence = computeConfidence(l1.passRate, trials.length, l2.cohenD == null ? 0 : Math.abs(l2.cohenD));
    let dormant_reason = null;
