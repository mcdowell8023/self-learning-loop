...
import { closeSync, fsyncSync, openSync, readFileSync, renameSync, unlinkSync, writeSync, existsSync, } from 'node:fs';
import { randomBytes } from 'node:crypto';
/** Strict SHA-256 check — 64 lowercase hex chars. */
const SHA256_RE = /^[a-f0-9]{64}$/;
export function assertSha256(hash) {
    if (!SHA256_RE.test(hash)) {
        throw new Error(`Invalid SHA-256 marker hash: expected 64 lowercase hex chars, got "${hash}"`);
    }
}
/** Build start / end marker lines for a given sha256 hash. */
export function markerLines(hash) {
    assertSha256(hash);
    return {
 ...
 */
function blockRegex(hash) {
    assertSha256(hash);
    return new RegExp(`<!--\\s*graduated:sha256:${hash}\\s+start\\s*-->[\\s\\S]*?<!--\\s*graduated:sha256:${hash}\\s+end\\s*-->`, 'g');
}
/** True iff the given file already contains a block for this hash. */
export function hasMarkerBlock(filePath, hash) {
...
export function readMarkerBlock(filePath, hash) {
    if (!existsSync(filePath))
        return null;
    const content = readFileSync(filePath, 'utf-8');
 ...
 */
export function atomicWrite(target, newContent) {
 ...
 */
export function injectMarkerBlock(target, hash, body, opts = {}) {
    assertSha256(hash);
 ...
 */
export function removeMarkerBlock(target, hash) {
    assertSha256(hash);
