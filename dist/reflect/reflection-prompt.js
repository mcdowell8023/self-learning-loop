...
export function formatEvents(events, opts = {}) {
    ...
    return slice
        ...
        const ts = e.timestamp instanceof Date
            ...
            : String(e.timestamp);
        const content = e.content.length > truncate
...
// 主入口
// ---------------------------------------------------------------------------
export function buildReflectPrompt(params) {
    const template = params.template ?? DEFAULT_REFLECT_TEMPLATE;
    const env = params.env;
    const envExtensions = env.extensions && Object.keys(env.extensions).length > 0
        ? 'Extensions: ' + safeMeta(env.extensions)
        : '';
    const existing = params.existingStrategies && params.existingStrategies.length > 0
        ? params.existingStrategies.map((s) => `- ${s}`).join('\n')
        : '(无)';
    const categories = params.problemCategories && params.problemCategories.length > 0
