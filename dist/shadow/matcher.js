...
export const DEFAULT_MATCHER_CONFIG = {
    minKeywordHits: 2,
    minKeywordLen: 3,
    maxKeywords: 20,
};
 ...
 *   - 去重、去停用词、去过短词
 */
export function extractKeywords(text, config = DEFAULT_MATCHER_CONFIG) {
    if (!text)
        return [];
    ...
    for (const tok of all) {
        if (tok.length < config.minKeywordLen)
            continue;
        ...
        seen.add(tok);
        out.push(tok);
        if (out.length >= config.maxKeywords)
            break;
    }
    return out;
}
...
// 粗筛：scope 匹配
// ---------------------------------------------------------------------------
export function coarseMatch(scope, events) {
    if (scope === 'general')
        return { passed: true };
    if (scope === 'skill') {
        const hit = events.some((e) => (typeof e.metadata?.skill === 'string' && e.metadata.skill.length > 0) ||
    ...
    if (scope.startsWith('tool:')) {
        ...
        const hit = events.some((e) => e.type === 'tool_call' &&
    ...
    if (scope.startsWith('role:')) {
        ...
        const hit = events.some((e) => e.metadata?.role === roleName ||
...
// 细筛：关键词 substring 匹配
// ---------------------------------------------------------------------------
export function fineMatch(keywords, events, config = DEFAULT_MATCHER_CONFIG) {
 ...
 */
export function matchSession(strategy, events, config = DEFAULT_MATCHER_CONFIG) {
    if (events.length === 0) {
        return {
            matched: false,
            coarsePassed: false,
