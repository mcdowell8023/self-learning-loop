...
import { DEFAULT_MATCHER_CONFIG, matchSession, } from './matcher.js';
...
export class ShadowRunner {
    ...
    buildTrial(args) {
        ...
        const turns = events.filter((e) => e.type === 'user_message' || e.type === 'assistant_message').length;
        const errors = events.filter((e) => e.type === 'error').length;
        const tokenUsage = events.reduce((sum, e) => {
            const t = e.metadata?.token_usage ?? 0;
