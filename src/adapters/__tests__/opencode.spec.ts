...
function createMockDb(dbPath: string) {
...
function insertSession(db: Database.Database, id: string, timeCreated: number, timeUpdated?: number) {
  db.prepare(
    `INSERT INTO session (id, project_id, title, slug, directory, version, time_created, time_updated)
     VALUES (?, 'proj1', 'Test Session', 'test', '/tmp', '1', ?, ?)`,
  ).run(id, timeCreated, timeUpdated ?? timeCreated);
}

function insertMessage(db: Database.Database, id: string, sessionId: string, timeCreated: number, data: object) {
  db.prepare(
    `INSERT INTO message (id, session_id, time_created, time_updated, data) VALUES (?, ?, ?, ?, ?)`,
  ).run(id, sessionId, timeCreated, timeCreated, JSON.stringify(data));
}

function insertPart(db: Database.Database, id: string, messageId: string, sessionId: string, timeCreated: number, data: object) {
  db.prepare(
...
describe('OpencodeAdapter', () => {
  ...
  it('extractEvents converts text parts to SessionEvent correctly', async () => {
    ...
    const events: import('../../kernel/schemas/session.js').SessionEvent[] = [];
    for await (const e of adapter.extractEvents(ref)) events.push(e);
