...
export function computeContentHash(event: Pick<SessionEvent, 'type' | 'content'>): string {
...
export function envRichnessScore(env: Partial<EnvFingerprint> | undefined | null): number {
  if (!env) return 0;
  let score = 0;
...
export function preferRicherEnv(existing: EventWithEnv, incoming: EventWithEnv): EventWithEnv {
...
export function deduplicateEvents(
  events: EventWithEnv[],
  seen: Map<string, { date: Date; event: EventWithEnv }> = new Map(),
): EventWithEnv[] {
  const result: EventWithEnv[] = [];
...
type OpenClawEventType = SessionEvent['type'];
...
export class OpenClawAdapter implements RuntimeAdapter {
  ...
  readonly extendedEventTypes = ['thinking', 'model_change', 'session', 'custom'];
  ...
  private loadPersistedOffsets(): void {
    try {
  ...
  async listNewSessions(since: Date): Promise<SessionRef[]> {
    ...
    try {
      ...
      throw new AdapterError(
        `failed to list sessions in ${this.sessionsDir}`,
        this.id,
        err,
      );
    }
    ...
    for (const fullPath of files) {
      try {
        ...
        this.emitAudit('stat_failed', { path: fullPath, error: (err as Error).message });
      }
    }
    return refs;
  }
        ...
        try {
          ...
          for (const s of sub) {
            ...
            if (s.isFile() && s.name.endsWith('.jsonl')) {
              ...
              for (const f of leaf) {
                if (f.endsWith('.jsonl')) out.push(join(sp, f));
              }
            }
          }
        } catch {
  ...
  private *toSessionEvents(
    ...
    ref: SessionRef,
  ): IterableIterator<SessionEvent> {
    const ts = this.parseTimestamp(raw);
    const rawType = typeof raw['type'] === 'string' ? (raw['type'] as string) : 'system';
    ...
    if (rawType === 'message') {
      ...
      const role = typeof message?.['role'] === 'string' ? (message['role'] as string) : 'system';
      ...
      for (const partUnknown of content) {
        ...
        const partType = typeof part['type'] === 'string' ? (part['type'] as string) : 'text';
        if (partType === 'text') {
          yield this.buildEvent({
            type: this.mapRoleToType(role),
            timestamp: ts,
            content: typeof part['text'] === 'string' ? (part['text'] as string) : '',
          ...
          yield this.buildEvent({
            type: 'thinking',
            timestamp: ts,
            content: typeof part['thinking'] === 'string' ? (part['thinking'] as string) : '',
    ...
    yield this.buildEvent({
      type: rawType as OpenClawEventType,
      timestamp: ts,
      content,
      metadata: { ...commonMeta },
      raw,
  ...
  private mapRoleToType(role: string): OpenClawEventType {
    switch (role) {
      case 'user':
      case 'human':
        return 'user_message';
  ...
  private parseTimestamp(raw: Record<string, unknown>): Date {
  ...
  private stringifySystemEvent(raw: Record<string, unknown>): string {
