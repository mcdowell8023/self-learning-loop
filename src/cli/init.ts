...
export function parseInitArgs(argv: string[]): ParsedInit {
...
export async function runInit(opts: InitRunOptions): Promise<InitResult> {
  ...
  let store: ReturnType<typeof openCandidateStore> | null = null;
  ...
  try {
    ...
    if (!exists || parsed.force) createdFiles.push(configPath);
    ...
    if (!mode) throw new Error('failed to establish SQLite journal mode');
    store.close();
    store = null;
    ...
    out(
      `✓ Initialised learn/ workspace at ${learnDir}\n` +
        `  • db:     ${dbPath}\n` +
        `  • config: ${configPath}\n` +
        `  • audit:  ${auditPath}\n`,
    );
    ...
    const msg = `error: init failed: ${(e as Error).message}`;
    err(msg + '\n');
