...
export async function runLearn(opts: LearnRunOptions): Promise<LearnResult> {
...
async function runConfigGroup(argv: string[], ctx: GroupCtx): Promise<LearnResult> {
  const [sub, ...rest] = argv;
  ...
  if (sub === 'reload') {
    ...
    if (result.ok) {
      ctx.out(
        `✓ config reloaded (changed sections: ${
          result.changed_sections.length > 0 ? result.changed_sections.join(', ') : 'none'
        })\n`,
      );
      return { exitCode: 0, command: 'config reload', subResult: result };
    } else {
      ctx.err(
        `error: config reload failed [${result.error_code}]: ${result.message}\n`,
      );
...
if (isMain) {
  runLearn({ argv: process.argv.slice(2) }).then(
    (r) => {
