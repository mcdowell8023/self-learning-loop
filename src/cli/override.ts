...
export async function runOverride(opts: OverrideRunOptions): Promise<OverrideResult> {
  ...
  try {
    ...
    if (command === 'force-graduate') {
      ...
      if (fromState === 'graduated' || fromState === 'rejected') {
        const msg =
          `error: illegal override: cannot force-graduate candidate ${candidateId} ` +
      ...
      if (fromState === 'retired' || fromState === 'rejected') {
        const msg =
          `error: illegal override: cannot force-retire candidate ${candidateId} ` +
    ...
    out(
      `✓ ${action}: ${candidateId}  ${fromState} → ${toState}` +
    ...
    const msg = `error: ${(e as Error).message ?? String(e)}`;
    err(msg + '\n');
...
type ParsedErr = { kind: 'error'; code: number; message: string };
type ParsedHelp = { kind: 'help' };
...
function appendAudit(path: string, event: unknown): void {
