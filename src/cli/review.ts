...
export async function runReview(opts: ReviewRunOptions): Promise<ReviewResult> {
...
function reviewList(
  ...
  if (stateFilter) {
    filter.state = stateFilter as any;
  } else {
    // Default: show review-related states
    filter.state = ['pending', 'reviewing', 'validating'] as any;
  }

  const candidates = store.list(filter as any);
