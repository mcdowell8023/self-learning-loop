...
export async function writeOrAppendDailyReport(
  ...
  const existingRunNumbers = Array.from(existing.matchAll(/^## Run #(\d+)/gm)).map(match => Number(match[1]));
  const nextRun = (existingRunNumbers.length > 0 ? Math.max(...existingRunNumbers) : 0) + 1;
  const firstRunIndex = existing.indexOf('## Run #');
  const existingRunSections = firstRunIndex >= 0 ? existing.slice(firstRunIndex).trim() : '';
  const nextSection = renderRunSection({ ...latestRun, generatedAt: runMeta.generatedAt }, nextRun);
  const header = renderHeader(reportData, { ...latestRun, generatedAt: runMeta.generatedAt }, nextRun);
  const tail = [
    existingRunSections,
    nextSection,
    '🤖 Generated by self-learning-loop v1.1.0-alpha.5',
