...
export async function reviewMetadata(
  ...
  if (typeof s.scope === 'string' && s.scope.trim() !== '') {
    ...
    if (!ok) errors.push(`invalid_scope_format:${s.scope}`);
  }
  ...
  const hasCreatedByTag =
    Array.isArray(s.tags) && s.tags.some((t) => typeof t === 'string' && t.startsWith('created_by:'));
