...
function collectTextBlobs(c: Candidate): string[] {
  const out: string[] = [];
  ...
  for (const inst of c.instances) {
    ...
    for (const a of inst.assertions ?? []) {
      if (typeof a.command === 'string') out.push(a.command);
      if (typeof a.description === 'string') out.push(a.description);
      if (Array.isArray(a.command_allowlist)) out.push(a.command_allowlist.join(' '));
    }
  }
  return out;
}

function collectPaths(c: Candidate): string[] {
  const out: string[] = [];
  for (const inst of c.instances) {
    for (const f of inst.files_touched ?? []) out.push(f);
  }
  return out;
}
...
export async function reviewSafety(
  ...
  const hits: Array<{ type: string; value: string; where: string }> = [];
