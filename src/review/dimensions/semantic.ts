...
export async function reviewSemantic(
  ...
  if (cfg.requireVerifiableAssertion) {
    const hasRegistered =
      ...
      candidate.instances.some((inst) =>
        (inst.assertions ?? []).some((a) =>
          deps.assertionRegistry ? deps.assertionRegistry.hasType(a.type) : false,
        ),
      );
    if (!hasRegistered) {
    ...
    if (!any) warnings.push('no_assertion_defined');
  }

  // 4) 相似度去重（短路：质量问题优先）
  let maxSim = 0;
  let dupStrategyId: string | undefined;
  if (errors.length === 0 && deps.strategyLookup) {
    ...
    for (const other of existing) {
      // 跳过自己
      if (other.strategy_id === s.strategy_id) continue;
